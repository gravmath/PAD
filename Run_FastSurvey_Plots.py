...
for i in range(60):
    ...
    for N in range(len(e)):
        ...
        Plasma_Plotter.make_Et_panel      (fig,axs[4],obs,'DES',e_t,e_E,e_omni,e_scpot)
        Plasma_Plotter.make_Et_panel      (fig,axs[5],obs,'DIS',i_t,i_E,i_omni,i_scpot)
        Plasma_Plotter.make_Bvector_panel (axs[6],obs,e_t,e_B)
        Plasma_Plotter.add_info_box(fig,'%s-%s-%s'%(curr_day,curr_month,curr_year),[-0.02,0.017,0.1,0.1])
        Plasma_Plotter.pos_on_time_axis(fig,axs[6],cursor,fpi_prd1,obs,mec_mode,descriptor,curr_year,curr_month,curr_day)
        fig.savefig(fpi_prd1+'/fpishare/Conrad/test/FastSurvey_plot_%s_%s_%s_%s_%s.png'%(obs,curr_year,curr_month,curr_day,N))
        plt.close()