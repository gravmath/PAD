...
#
###############################################################################
def unpack_dist_CDF(cdf_fh,obs,species,ver,type,corrections_on,correction_override=0.0):
    dist           = cdf_fh['dist']
    ...
    if corrections_on == 0:
        ...
        corrected_Dist   = subtract_internal_photoelectrons(cdf_fh,uncorrected_Dist,obs,species,correction_override)
    
    dist     = {'Dist'   : corrected_Dist,
...
#
###############################################################################    
def subtract_internal_photoelectrons(cdf_fh,raw_Dist,obs,species,correction_override):
...
#
###############################################################################
def fetch_magnetic_field(cdf_fh,obs,species,source="DEBUG"):
...
#
###############################################################################
def calculate_incoming_particle_directions(params):
...
#
###############################################################################
def load_e_data(cdf_fh,obs,species,ver,corrections_on,correction_override = 0,source="DEBUG"):
    
    dist, parms         = unpack_dist_CDF(cdf_fh,obs,species,ver,corrections_on,correction_override)
    bfield              = fetch_magnetic_field(cdf_fh,obs,species,source)
