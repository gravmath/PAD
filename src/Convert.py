...
############################################################################
def convert_GCI_to_GSE(epoch_dt):
    S, V  = SunEph.CalcSun_Low(epoch_dt)
    mag_S = np.sqrt(S.dot(S))
    mag_V = np.sqrt(V.dot(V))
    epole = np.cross(S,V)/mag_S/mag_V
    X_hat = S/mag_S
    Y     = np.cross(epole,X_hat)
...
############################################################################    
def convert_ECEF_to_GCI(epoch_dt):
    gha   = equation_of_time.calculate_GMST(epoch_dt)
    cos_g = np.cos(gha)
    sin_g = np.sin(gha)
...
############################################################################    
def convert_GSE_to_GSM(epoch_dt):
    A_gse_gci     = convert_GCI_to_GSE(epoch_dt)
    S, V          = SunEph.CalcSun_Low(epoch_dt)
    D             = convert_ECEF_to_GCI(epoch_dt).dot(dipole_ECEF)
...
############################################################################
def convert_GSM_to_ECEF(epoch_dt):
    T_GCI_ECEF = convert_ECEF_to_GCI(epoch_dt)
    T_GSE_GCI  = convert_GCI_to_GSE(epoch_dt)
    T_GSM_GSE  = convert_GSE_to_GSM(epoch_dt)
...
############################################################################
def convert_GCI_to_SM(epoch_dt):
    S, V  = SunEph.CalcSun_Low(epoch_dt)
    D     = convert_ECEF_to_GCI(epoch_dt).dot(dipole_ECEF)
...
############################################################################
def convert_GCI_to_GSM(epoch_dt):
    return convert_GSE_to_GSM(epoch_dt).dot(convert_GCI_to_GSE(epoch_dt))
